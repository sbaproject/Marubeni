...
@section('content')
  ...
  <div class="alert alert-danger alert-dismissible fade show">
        <h6 class="alert-msg"><i class="icon fas fa-ban"></i>{{ __('label.flow_can_not_update') }}</h6>
    ...
    <div class="row">
        <div class="col-12 col-lg-10">
            ...
            <div class="form-group row">
                ...
                <input type="text" class="form-control" value="{{ $flow->flow_name }}" @if(!$canEdit) disabled="disabled" @endif name="approval_flow_name" placeholder="{{ __('label.flow_approval_flow_name') }}">
                </div>
            </div>
            <div class="form-group row">
            ...
            <div class="form-group row form-trip" @if(strval($flow->form_id) !== '2') style="display: none" @endif> 
              ...
              <div class="col-lg-9">
                <div class="form-check-inline"><label class="form-check-label"><input type="radio" value="3" @if(!$canEdit) disabled="disabled" @endif name="trip" @if(strval($flow->position) === '3') checked="checked" @endif>{{ __('label.budget_economy_class') }}</label></div>
                <div class="form-check-inline"><label class="form-check-label"><input type="radio" value="4" @if(!$canEdit) disabled="disabled" @endif  name="trip" @if(strval($flow->position) === '4') checked="checked" @endif>{{ __('label.budget_business_class') }}</label></div>
              </div>
            </div>
            <div class="form-group row form-entertaiment" @if(strval($flow->form_id) !== '3') style="display: none" @endif>
              <label class="col-lg-3 col-form-label text-center">{{ __('label.flow_type') }}</label>
              <div class="col-lg-9">
                <div class="form-check-inline"><label class="form-check-label"><input @if(!$canEdit) disabled="disabled" @endif class="typePosition" id="positionPO" type="radio" value="1" @if(strval($flow->position) === '1') checked="checked" @endif name="PO">{{ __('label.budget_po') }}</label></div>
                <div class="form-check-inline"><label class="form-check-label"><input @if(!$canEdit) disabled="disabled" @endif class="typePosition" id="positionNotPO" type="radio" value="2" name="PO" @if(strval($flow->position) === '2') checked="checked" @endif>{{ __('label.budget_not_po') }}</label></div>
              </div>
            </div> 
            <div class="form-group row form-entertaiment form-po" @if(strval($flow->form_id) !== '3' || strval($flow->position) === '2') style="display: none" @endif>
              ...
              <select id="cbxBudgetTypePO" name="budget_type_po" class="form-control select2" @if(!$canEdit) disabled="disabled" @endif>
                 <option value="0" @if(strval($flow->budget_type_compare) === '0') selected @endif>{{ __('label.flow_less_or_equal_than') }} {{ $budgetPO }}</option>
                 <option value="1" @if(strval($flow->budget_type_compare) === '1') selected @endif>{{ __('label.flow_greater_than') }} {{ $budgetPO }}</option>
              </select>
              </div>
            </div> 
            <div class="form-group row form-entertaiment form-not-po" @if(strval($flow->form_id) !== '3' || strval($flow->position) === '1') style="display: none" @endif>
              ...
              <select id="cbxBudgetTypeNotPO" name="budget_type_not_po" class="form-control select2" @if(!$canEdit) disabled="disabled" @endif>
                 <option value="0" @if(strval($flow->budget_type_compare) === '0') selected @endif>{{ __('label.flow_less_or_equal_than') }} {{ $budgetNotPO }}</option>
                 <option value="1" @if(strval($flow->budget_type_compare) === '1') selected @endif>{{ __('label.flow_greater_than') }} {{ $budgetNotPO }}</option>
              </select>
              </div>
            </div>                      
            <div class="form-group row">
            ...
            <div class="border border-secondary p-3 wrap-step-1 section-step-{{ $step->step_type }}">
                    ...
                    <div class="block-add-approver-{{ $start_step }}">
                          <button @if(!$canEdit) disabled="disabled" @endif type="button" data-step="{{ $start_step }}" data-index="{{$step_index + 1}}" class="btn-add-approver btn btn-outline-dark pt-0 pb-0 pl-3 pr-3">+ {{ __('label.flow_add') }}</button>
                    </div>
                    </div>
                ...
                <div class="section-approver approver-{{ $step->step_type }}-{{$step_index + 1}}">    
                  <div class="d-flex justify-between">
                    <div class="text-muted">{{ __('label.flow_approver') }} <span class="title-approver">{{ $step_index + 1}}</span></div>
                    <div>
                      <button @if(!$canEdit) disabled="disabled" @endif type="button" data-step="{{ $step->step_type }}" data-index="{{$step_index + 1}}" class="btn-del-approver btn btn-danger btn-sm pt-0 pb-0 pl-3 pr-3 mb-1">{{ __('label.flow_delete') }}</button>
                    </div>
                         
                  </div>
                @else
                  <div class="text-muted">{{ __('label.flow_approver') }} {{$step_index + 1}}</div>
                @endif
                  <table class="table table-bordered table-sm">
                      <tr>
                          ...
                          <td class="text-left">
                              ...
                              @if($step_index > 0)  
                               {{ __('label.flow_destination_invalid_end') }}
                              @else 
            ...
            @endforeach   
              <div class="block-add-approver-{{ $start_step }}">
                    <button @if(!$canEdit) disabled="disabled" @endif type="button" data-step="{{  $start_step }}" data-index="{{ $step_index }}" class="btn-add-approver btn btn-outline-dark pt-0 pb-0 pl-3 pr-3">+ {{ __('label.flow_add') }}</button>
              </div>
            </div>
            </div>
            </div>
            <div class="mt-3 block-add-step" style="display: none"><button @if(!$canEdit) disabled="disabled" @endif type="button" data-step="1" data-index="0" class="btn-add-step btn btn-outline-dark pt-1 pb-1 pl-3 pr-3">+ {{ __('label.flow_step') }}</button></div>


            <div class="mt-5 mb-5 text-center">
                <button @if(!$canEdit) disabled="disabled" @endif type="button" class="btn-submit-flow btn btn-danger pt-1 pb-1 mr-4 col-5 col-sm-2 col-md-4 col-lg-2"><i class="nav-icon far fa-check-circle"></i> {{ __('label.flow_update') }}</button>
