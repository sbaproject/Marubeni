...
<li class="breadcrumb-item active">{{ __('label.menu_department_edit') }}</li>
@endsection
    ...
    <div class="invoice p-3">
        <div class="card-body">
            <div class="search-content">
                <form action="{{ route("admin.department.update",$iddepartment)}}" method="POST">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9">
                            <input id="name" type="text" class="form-control @error('name') is-invalid @enderror"
                                name="name" value="{{ old('name', $department->name) }}" placeholder="{{ __('validation.attributes.user.name') }}" autocomplete="off" autofocus>
                            @error('name')
                        ...
                        <div class="col-lg-9">
                            <textarea id="memo" name="memo" rows="4"
                                class="form-control @error('memo') is-invalid @enderror"
                                    placeholder="{{ __('validation.attributes.memo') }}" autocomplete="off" autofocus>{{ old('memo', $department->memo) }}</textarea>
                            @error('memo')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
                    ...
                    <div class="mt-5 mb-5 text-center">
                        ...
                        <button type="submit" class="btn bg-gradient-success">
                            <i class="nav-icon far fa-check-circle"></i> {{ __('label.button_update') }}</button>
                        {{-- Cancel --}}
