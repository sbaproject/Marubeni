...
<li class="breadcrumb-item active">{{ __('label.menu_applicant_edit') }}</li>
@endsection
    ...
    <div class="invoice p-3">
        <div class="card-body">
            <div class="search-content">
                <form action="{{ route("admin.applicant.update",$idapplicant)}}" method="POST">
                    @csrf
                            ...
                            <fieldset id="location" class="@error('location') form-control is-invalid @enderror">
                                ...
                                <label class="radio-inline com_title col-form-label">
                                    <input type="radio" name="location" value="{{ $value }}"
                                        {{ old('location', $applicant->location ) == $value ? 'checked' : '' }}>
                                    {{ __('label.'.$key) }}
                                </label>
                                @endforeach
                            </fieldset>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-lg-9">
                            <select id="department_id" name="department_id"
                                ...
                                <option value="{{ $item->id }}" {{ old('department_id' , $applicant->department_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}</option>
                                @endforeach
                            </select>
                            @error('department_id')
                        ...
                        <div class="col-lg-9">
                            <div class="col-md-6" style="padding-left:0">
                                <select id="role" name="role" class="form-control @error('role') is-invalid @enderror">
                                    ...
                                    <option value="{{ $value }}" {{ old('role', $applicant->role) == $value ? 'selected' : '' }}>
                                        {{ $key }}</option>
                                    @endforeach
                                </select>
                                @error('role')
