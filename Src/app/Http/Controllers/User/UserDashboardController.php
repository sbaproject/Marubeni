...
class UserDashboardController extends Controller
{
    public function index($id, Request $request)
    {
        ...
        if (!empty($data['dataDateFrom']) && !empty($data['dataDateTo'])) {
            ...
            if ($id == config('const.application.status.all')) {
                $list_application =  Application::where('created_by', $userId)->where('status', '!=', config('const.application.status.draft'))->orderBy('id', 'DESC')->paginate(5);
            } else if ($id == config('const.application.status.applying')) {
                $list_application =  Application::where('created_by', $userId)->where('status', config('const.application.status.applying'))->orderBy('id', 'DESC')->paginate(5);
            } else if ($id == config('const.application.status.approvel')) {
                $list_application =  Application::where('created_by', $userId)->whereBetween('status', [1, 98])->orderBy('id', 'DESC')->paginate(5);
            } else if ($id == config('const.application.status.declined')) {
                $list_application =  Application::where('created_by', $userId)->where('status', config('const.application.status.declined'))->orderBy('id', 'DESC')->paginate(5);
            } else if ($id == config('const.application.status.reject')) {
                $list_application =  Application::where('created_by', $userId)->where('status', config('const.application.status.reject'))->orderBy('id', 'DESC')->paginate(5);
            } else if ($id == config('const.application.status.completed')) {
                $list_application =  Application::where('created_by', $userId)->where('status', config('const.application.status.completed'))->orderBy('id', 'DESC')->paginate(5);
            }
