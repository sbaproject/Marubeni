...
class Common
{
	...
	public static function getRoutePreviewApplication($appId, $formId)
	{
		if ($formId === config('const.form.leave')) {
			...
			$busi2 = Businesstrip2::where('application_id', '=', $appId)->first();
			if ($busi2 !== null) {
				return route('user.business2.show', $appId);
			}
			return route('user.business.preview', $appId);
		} elseif ($formId === config('const.form.entertainment')) {

			$enter2 = Entertainment2::where('application_id', '=', $appId)->first();
			if ($enter2 !== null) {
				return route('user.entertainment2.show', $appId);
			}
